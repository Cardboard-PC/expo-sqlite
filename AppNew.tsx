...
export default function AppNew() {
  ...
  // return a List of react components (from an input list of SQL records)
  // - `key={index}` is not related to the SQL record, but to the react component generated from a map,
  ...
  // https://youtu.be/1kSLd9oQX7c?t=717
  // I'm still confused what their is a lambda function here
  const addNames = () => {
    // SQL Callback (param 1)
    db.transaction(tx => {
      // SQL Statement (param 1)
      tx.executeSql('INSERT INTO names (name) values (?)',
      // SQL Args (param 2)
      [currentName],
      // Success Callback (param 3)
      (txObj, resultSet) => {
        let existingNames = [...names];
        existingNames.push({ id: resultSet.insertId, name: currentName});
        setNames(existingNames);
        setCurrentName(undefiend);
      },
      // Error Callback (param 4)
      (txObj, error) => console.log(error)
    );
    })
  }
  ...
  const deletName = (id) => {
    db.transaction(tx => {
      tx.executeSql(
        'DELETE FROM name WERE id = ?',
        [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAddected > 0) {
